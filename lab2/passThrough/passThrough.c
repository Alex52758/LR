...
--*/
{
    ...
    status = FltGetFileNameInformation(
        ...
        &NameInfo);
    UNICODE_STRING required_extension = RTL_CONSTANT_STRING(L"ryzhenkov");
    if (!NT_SUCCESS(status))
    {
        ...
        RtlEqualUnicodeString(
            ...
            &NameInfo->Extension, \
            FALSE)) {
        DbgPrint("[+] NameInfo: %wZ extention: %wZ perentDir: %wZ volume: %wZ \n",
            NameInfo->Name,
            NameInfo->Extension,
            NameInfo->ParentDir,
            NameInfo->Volume);
...
--*/
{
    //DbgPrint("*** PtPostOperationPassThrough()");
    ...
    // условие, которое срабатывает в данный момент
    // драйвер обрабатывает данные от файла с определенным именем,
    // или расширением, или от определенного приложения
    //1. из параметром определить какому файлу или расширению или программе
    // принадлежат данные
    // необходимые локальные переменные
    NTSTATUS status; //объявляем переменную статуса
    PFLT_FILE_NAME_INFORMATION NameInfo = NULL;
    // извлекаем из IRP-запроса Data имя файла и расширение и кладем in NameInfo
    status = FltGetFileNameInformation(
        Data,
        FLT_FILE_NAME_NORMALIZED |
        FLT_FILE_NAME_QUERY_DEFAULT,
        &NameInfo); //передаем в неймИнфо ссылку на структуру
    UNICODE_STRING required_extension = RTL_CONSTANT_STRING(L"ryzhenkov");
    if (!NT_SUCCESS(status))
    {
        //…
        ...
        // сравниваем расширение не учитывая регистр
        RtlEqualUnicodeString(
            &required_extension,
            &NameInfo->Extension,
            FALSE)) {  //2. Если данные принадлежат определенному файлу или 
        //программе - срабатывает данная  ветка 

        //3. Определить, чтение или запись происходит в данный момент
        if (Data->Iopb->MajorFunction == IRP_MJ_WRITE) { //шифрование
            DbgPrint("write");
            DbgPrint("write Buffer: %s\n", Data->Iopb->Parameters.Write.WriteBuffer);
            //DbgPrint(Data->Iopb->Parameters.Write.WriteBuffer);
            //4. здесь исп. сторонней библиотеки
            if (Data->Iopb->Parameters.Write.WriteBuffer) {
                uint8_t hexarray[1024];
                //функция memset позволяет заполнить весь массив hexarray 0
                memset(hexarray, 0, 1024);
                for (int i = 0; i < strlen(Data->Iopb->Parameters.Write.WriteBuffer); i++) {
                    // заполняем массив hexarray
                    hexarray[i] = (uint8_t)((char*)Data->Iopb->Parameters.Write.WriteBuffer)[i];
                }
                //http://base64.ru/ для генерации ключа 
                unsigned char k[] = "0J/QsNC90LjQvQ==";    //16 битный ключ
                uint8_t* key = (uint8_t*)k;
                uint8_t iv[] = { 0x77, 0x53, 0x5f, 0x43, 0x6e, 0x77, 0x67, 0x75, 0x63, 0x33, 0x34, 0x59, 0x6e, 0x64, 0x24, 0x25 };
        ...
        else if (Data->Iopb->MajorFunction == IRP_MJ_READ) { //расшифрование
            DbgPrint("read");
            ...
            if (strlen((char*)Data->Iopb->Parameters.Read.ReadBuffer) != 0) {
                ...
                unsigned char k[] = "0J/QsNC90LjQvQ==";    //16 битный ключ
                uint8_t* key = (uint8_t*)k;               //указатель на ключ
                uint8_t iv[] = { 0x77, 0x53, 0x5f, 0x43, 0x6e, 0x77, 0x67, 0x75, 0x63, 0x33, 0x34, 0x59, 0x6e, 0x64, 0x24, 0x25 };
