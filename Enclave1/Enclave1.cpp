...
void getElementByIndex(char* buf, size_t len, size_t idx) {
    if (idx < 5) {
        ...
        memcpy(buf, data_ptr, strlen(data_ptr)); // Функция memcpy копирует num байтов первого блока памяти, 
        //на который ссылается указатель srcptr, во второй блок памяти, на который ссылается указатель destptr.
    }
    else {
        memset(buf, 0, strlen(table[0])); // Функция memset заполняет num байтов блока памяти, 
        //через указатель memptr. Код заполняемого символа передаётся в функцию через параметр val.
    }
    return;
}